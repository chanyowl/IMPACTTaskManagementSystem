...
import { reportingPromptTemplate } from '../prompts/reporting_prompt.js';
import { reviewerPromptTemplate } from '../prompts/reviewer_prompt.js';
import { reviserPromptTemplate } from '../prompts/reviser_prompt.js';
...
async function callOpenAI(systemPrompt: string, userContent: string): Promise<string> {
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-5.1', // Using the specifically requested model name
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: userContent }
      ],
      temperature: 0.2, // Lower temperature for more consistent, rule-abiding reporting output
    })
  });
...
/**
 * Reviews the draft generated by Gemini using OpenAI as a Senior DOST-PCIEERD Report Reader Specialist
 */
async function reviewDOSTNarrative(draft: string): Promise<string> {
  return callOpenAI(reviewerPromptTemplate, `=== DRAFT REPORT TO REVIEW ===\n\n${draft}`);
}

/**
 * Revises the draft using the feedback generated, acting as an IMPACT-NXT Report Revision Specialist
 */
async function reviseDOSTNarrative(draft: string, feedback: string): Promise<string> {
  return callOpenAI(reviserPromptTemplate, `### ORIGINAL DRAFT REPORT:\n\n${draft}\n\n### REVIEWER FEEDBACK:\n\n${feedback}`);
}

/**
 * Generate a specific DOST-format narrative
 */
async function generateDOSTNarrative(reportData: any): Promise<{ narrative: string; revisionLog?: string }> {
  try {
    ...
    // Use compiled TS prompt template
    const promptTemplate = reportingPromptTemplate;
    ...
    const result = await model.generateContent(prompt);
    const response = await result.response;
    const initialDraft = cleanAIOutput(response.text());

    console.log('Step 2: Sending draft to OpenAI (GPT) for structural stress testing...');
    const feedback = await reviewDOSTNarrative(initialDraft);

    console.log('Step 3: Sending draft and feedback to OpenAI (GPT) for final polish and revisions...');
    const finalRevisedReport = await reviseDOSTNarrative(initialDraft, feedback);

    // Split the report and the revision log
    // Search for "REVISION LOG" (case-insensitive) preceded optionally by markdown headers
    const logRegex = /(?:#+\s*)?REVISION LOG/i;
    const splitIndex = finalRevisedReport.search(logRegex);

    let narrativePart = finalRevisedReport;
    let revisionLogPart = undefined;

    if (splitIndex !== -1) {
      narrativePart = finalRevisedReport.substring(0, splitIndex).trim();
      // Include the keyword in the revision log part so the title still shows up
      revisionLogPart = finalRevisedReport.substring(splitIndex).trim();
    }

    // Now clean the AI outputs
    return {
      narrative: cleanAIOutput(narrativePart),
      revisionLog: revisionLogPart ? cleanAIOutput(revisionLogPart) : undefined
    };
  } catch (error) {
    console.error('Error generating DOST narrative:', error);
    return { narrative: `Error generating DOST narrative: ${error instanceof Error ? error.message : String(error)} ` };
  }
}
...
export async function generateReportNarrative(
  reportData: any
): Promise<{ narrative: string; revisionLog?: string }> {
  try {
...
1. EXPLAIN THE PROGRESS: Summarize what was accomplished during this period, using specific task "updates" as evidence of subtask completion.
  ...
  TONE: Professional, objective, and authoritative.Avoid fluff.Focus on "What was done, by whom, and the impact on objectives".
    ...
    const result = await model.generateContent(prompt);
    const response = await result.response;
    return { narrative: cleanAIOutput(response.text()) };
  } catch (error: any) {
    console.error('Error in generateReportNarrative:', error);
    return { narrative: "Error generating narrative. Please review the raw statistics for compliance details." };
  }
}
