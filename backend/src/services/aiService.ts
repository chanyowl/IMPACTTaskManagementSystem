...
async function generateDOSTNarrative(reportData: any): Promise<string> {
  try {
    ...
    const prompt = `You are generating a DOST IMPACT-NXT Quarterly Narrative Report.
...
- Challenges: ${inputs.challenges || "None provided (Infer from task logs)"}
- Plans for Next Quarter: ${inputs.nextQuarterPlans || "None provided (Infer from 'Pending' tasks)"}
- Offline Accomplishments: ${inputs.actualAccomplishments || "None provided (Focus on dashboard data)"}
- Survey/Data Metrics: ${inputs.surveyMetrics || "None provided"}

*** DASHBOARD DATA EXTRACTED ***
${JSON.stringify(reportData.tasks.map((t: any) => {
      const history = reportData.taskAccomplishments?.[t.taskId] || [];
      return {
        TaskName: t.deliverable,
        Objective: t.objective, // Logic will match this ID to the System Context Objectives below
        Status: t.status,
...
*** EVIDENCE CONTENT(CRAWLED FROM URLs) ***
  ${JSON.stringify(reportData.crawledContent || {}, null, 2)}
...
${JSON.stringify(IMPACT_OBJECTIVES, null, 2)}
